...
def main():
    # 输入数据.
    pw, inequival_sites, event_gen_mode, control_step, repeat_run, nsteps, T_list, barriers, jumpfreqs, select_path = parse.read_input()
    t_control_list = np.ones(len(T_list))*1E30
    # 初始化事件.
    repeating_number, events_origin = preprocess_kmc.gen_events(select_path, event_gen_mode, inequival_sites)
