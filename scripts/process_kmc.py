...
def r_squared(slope, intecept, xdata, ydata):
    residuals = ydata - linear_func(xdata, slope, intecept)
...
def plot_tem_diffusivity_graph(slope, intecept, diffusion_coeffs, reciprocal_T_list):
    x_data = np.linspace(min(reciprocal_T_list)-0.1, max(reciprocal_T_list)+0.1, 1000)
    y_data = linear_func(x_data, slope, intecept)
...
def main():
    ...
    #slope, intecept, barrier, D0 = linear_fit(reciprocal_T_list, diffusion_coeffs)
    #Rsquared = r_squared(slope, intecept, reciprocal_T_list, diffusion_coeffs)
    #designated_T = 1000 / (285+273.15)
    #lg10D = designated_T*slope + intecept
    #D = 10**lg10D
    #print(D)
    T_list = [300, 500, 700, 1000, 1500, 2000, 2500, 3000]
    diffusion_coeffs = []
    ...
    reciprocal_T_list = 1000 / np.array(T_list)
    diffusion_coeffs = np.array(diffusion_coeffs)
    slope, intecept, barrier, D0 = linear_fit(reciprocal_T_list, diffusion_coeffs[:,3])
    Rsquared = r_squared(slope, intecept, reciprocal_T_list, diffusion_coeffs[:,3])
    plot_tem_diffusivity_graph(slope, intecept, diffusion_coeffs[:,3], reciprocal_T_list)
    with open ("process_kmc.dat", 'a') as output:
        output.write(f"Linear_fit: {slope}*x+{intecept}\n")
        print(f"Linear_fit: {slope}*x+{intecept}\n")
        output.write(f"R_squared: {Rsquared}\n")
        print((f"R_squared: {Rsquared}\n"))
        output.write(f"Diffusion_barrier: {barrier} eV\n")
        print(f"Diffusion_barrier: {barrier} eV\n")
        output.write(f"Prefactor: {D0}")
        print(f"Prefactor: {D0}")
