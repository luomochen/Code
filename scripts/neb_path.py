    ...
    try:
        if readmode == 'r':
            for i in range(image_number):
                ...
                if os.path.exists(f"{i:02d}/OUTCAR"):
                    image = vasp.read_vasp_out(f"{i:02d}/OUTCAR")
                elif os.path.exists(f"{i:02d}/CONTCAR"):
                    image = vasp.read_vasp(f"{i:02d}/CONTCAR")
                else:
                    image = vasp.read_vasp("{i:02d}/POSCAR")
                for ind, iatom in enumerate(image):
                    if iatom.symbol == interstitial_atom:
                        pft.extend(iatom)
            vasp.write_vasp("./diffpath.vasp", pft, direct=True)
            view_structure_file(pft)
            return 0
        else:
    ...
    try:
        ...
        # Use ase to read the OUTCAR.
            if i != 0 and i != image_number-1:
                image = vasp.read_vasp(f"{i:02d}/POSCAR")
                image_copy = copy.deepcopy(image)
                for ind, iatom in enumerate(image_copy):
                    if iatom.symbol == interstitial_atom:
                        del image_copy[ind]
                image_copy.extend(atoms[i])
                os.popen(f"cp {i:02d}/POSCAR {i:02d}/POSCAR.old")
                vasp.write_vasp(f"{i:02d}/POSCAR", image_copy, direct=True)
